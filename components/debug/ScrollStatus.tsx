...
const ScrollStatus = () => {
  ...
  return (
    <div className="fixed bottom-20 left-20 bg-black/75 text-white text-3xl p-6 rounded-md z-50 flex flex-col gap-8 ltr">
      <p>
        📍 <span className="text-muted-foreground dark:text-textSecondary">Active: </span>
        {activeSection ?? "None"}
      </p>
      <p>
        🎯 <span className="text-muted-foreground dark:text-textSecondary">Target: </span>
        {targetSection ?? "None"}
      </p>
      <p>
        🌀 <span className="text-muted-foreground dark:text-textSecondary">Scrolling: </span>
        {isScrolling ? "Yes" : "No"}
      </p>
    </div>
  );
