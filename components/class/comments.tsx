      ...
      <div key={comment.id} className="p-8 mb-6 bg-[#fafafa] shadow rounded-lg">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2 mb-6">
            ...
            {comment.verified && (
              <span className="text-xs bg-primary text-primary px-2 py-1 rounded">
                ✓ تایید شده
              </span>
            )}
              ...
              <div className="flex items-center gap-1">
                {Array.from({ length: 5 }).map((_, i) => (
                  <span
                    ...
                    className={clsx(
                      "text-sm",
                      i < comment.rating! ? "text-premium" : "text-muted-foreground"
                    )}
                  >
                    ★
                  </span>
          ...
          <div className="text-xs text-muted-foreground dark:text-textSecondary">
        ...
        <div className="flex items-center gap-4 mt-4 text-xs text-muted-foreground dark:text-textSecondary">
          <button
    ...
    <div className="mt-20 border-t">
      ...
      <div className="border-b w-full max-w-4xl">
        <h5 className="mt-8 mb-4 flex gap-3 items-center">
          <MdOutlineComment className="text-muted-foreground dark:text-textSecondary text-xl" />
          نظرات دوره
        </h5>
      </div>
      ...
      {isLoading && (
        <div className="max-w-4xl mt-10 text-center py-12">
          <div className="inline-block w-8 h-8 border-4 border-border dark:border-borderColor border-t-blue-600 rounded-full animate-spin" />
          <p className="mt-4 text-muted-foreground dark:text-textSecondary">در حال بارگذاری نظرات...</p>
        </div>
      )}

      {/* Error State */}
      {error && (
        <div className="max-w-4xl mt-10 text-center py-12 bg-destructive rounded-lg">
          <p className="text-destructive">خطا در بارگذاری نظرات</p>
          <p className="text-sm text-muted-foreground dark:text-textSecondary mt-2">
      ...
      {!isLoading && !error && comments.length === 0 && (
        <div className="max-w-4xl mt-10 text-center py-12 bg-muted dark:bg-darkBgHidden rounded-lg">
          <MdOutlineComment className="text-muted-foreground dark:text-textSecondary text-5xl mx-auto mb-4" />
          <p className="text-muted-foreground dark:text-textSecondary">هنوز نظری برای این دوره ثبت نشده است</p>
          <p className="text-sm text-muted-foreground dark:text-textSecondary mt-2">
      ...
      {!isLoading && !error && comments.length > 0 && (
        <div className="mt-10 max-w-4xl">
          <p className="text-sm text-muted-foreground dark:text-textSecondary mb-6">
