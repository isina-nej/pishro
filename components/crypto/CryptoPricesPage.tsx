...
function SortButton({ label, active, direction, onClick }: { label: string; active: boolean; direction: SortDirection; onClick: () => void }) {
  return <button type="button" onClick={onClick} className={`inline-flex items-center gap-1 text-right transition hover:text-cyan-200 ${active ? 'text-cyan-200' : ''}`} aria-label={`مرتب‌سازی بر اساس ${label}`}>{label}{active ? direction === 'asc' ? <ArrowUp className="h-3.5 w-3.5" /> : <ArrowDown className="h-3.5 w-3.5" /> : <ArrowUpDown className="h-3.5 w-3.5 opacity-50" />}</button>;
}
...
const assetTones: Record<string, string> = {
  BTC: 'from-orange-400 to-amber-600', ETH: 'from-indigo-300 to-violet-500',
  SOL: 'from-cyan-300 to-fuchsia-500', USDT: 'from-emerald-300 to-teal-600',
  BNB: 'from-yellow-200 to-orange-500', XRP: 'from-slate-200 to-blue-600',
};
...
function AssetIcon({ asset, large = false }: { asset: CryptoMarketAsset; large?: boolean }) {
  const tone = assetTones[asset.symbol] || 'from-sky-300 to-indigo-600';
  return (
    <span className={`flex shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br ${tone} text-slate-950 shadow-lg shadow-indigo-500/15 ${large ? 'h-14 w-14' : 'h-10 w-10'}`}>
...
function LoadingState() {
  return <div className="flex min-h-[420px] flex-col items-center justify-center gap-4 text-slate-400"><Loader2 className="h-8 w-8 animate-spin text-cyan-300" /><p className="text-sm">در حال دریافت داده‌های بازار...</p></div>;
}
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#07111f] text-white" dir="rtl">
      ...
      <div className={`relative mx-auto max-w-[1440px] px-4 pb-12 sm:px-6 lg:px-10 ${admin ? 'pt-5 lg:pt-8' : 'pt-20 md:pt-28'}`}>
        <div className="mb-7 flex items-center justify-between gap-4 rounded-3xl border border-white/10 bg-white/[0.055] px-4 py-3 shadow-2xl shadow-black/20 backdrop-blur-2xl sm:px-5">
          <div className="flex items-center gap-3"><div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-gradient-to-br from-cyan-300 to-blue-600 shadow-lg shadow-cyan-500/20"><CandlestickChart className="h-5 w-5 text-slate-950" /></div><div><p className="text-sm font-black tracking-tight text-white">پیشرو / بازارها</p><p className="hidden text-[10px] text-slate-400 sm:block">داده زنده بازار دارایی‌های دیجیتال</p></div></div>
          <div className="flex items-center gap-2 text-xs text-slate-300"><button type="button" onClick={() => loadMarket(true)} disabled={refreshing} className="flex items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.06] px-3 py-1.5 transition hover:bg-white/10 disabled:opacity-50"><RefreshCw className={`h-3.5 w-3.5 ${refreshing ? 'animate-spin' : ''}`} />به‌روزرسانی</button><span className="hidden items-center gap-1.5 rounded-full border border-emerald-300/20 bg-emerald-300/10 px-3 py-1.5 sm:flex"><span className="h-1.5 w-1.5 animate-pulse rounded-full bg-emerald-300" />{error ? 'آخرین داده موجود' : 'بازار فعال'}</span></div>
        </div>

        {loading && !market ? <LoadingState /> : !market ? <div className="rounded-3xl border border-rose-300/20 bg-rose-300/10 p-10 text-center"><p className="text-rose-200">{error || 'اطلاعات بازار در دسترس نیست.'}</p><button type="button" onClick={() => loadMarket(true)} className="mt-5 rounded-xl bg-white px-4 py-2 text-sm font-bold text-slate-950">تلاش دوباره</button></div> : <>
          {error && <div className="mb-5 rounded-2xl border border-amber-300/20 bg-amber-300/10 px-4 py-3 text-xs text-amber-100">به‌روزرسانی جدید ناموفق بود؛ آخرین داده دریافت‌شده نمایش داده می‌شود.</div>}
          <section className="mb-6 grid gap-5 xl:grid-cols-[1.35fr_.65fr]">
            <div className="relative min-h-[290px] overflow-hidden rounded-[2rem] border border-white/10 bg-gradient-to-br from-[#12253b]/95 via-[#10223a]/80 to-[#10152d]/90 p-6 shadow-2xl shadow-cyan-950/25 sm:p-9">
              <div className="relative z-10 max-w-xl"><div className="mb-5 inline-flex items-center gap-2 rounded-full border border-cyan-200/20 bg-cyan-200/10 px-3 py-1.5 text-[11px] font-semibold text-cyan-100"><Zap className="h-3.5 w-3.5 text-cyan-300" />نبض زنده بازار</div><h1 className="max-w-lg text-3xl font-black leading-[1.35] tracking-tight text-white sm:text-5xl">تصمیم‌های بهتر، با یک نگاه به <span className="bg-gradient-to-l from-cyan-200 via-white to-indigo-300 bg-clip-text text-transparent">بازار</span></h1><p className="mt-4 max-w-md text-sm leading-7 text-slate-300 sm:text-base">قیمت جهانی، نمودار هفت‌روزه و ارزش ریالی رمزارزهای مهم را یکجا دنبال کنید.</p></div>
            ...
            {bitcoin && <div className="relative overflow-hidden rounded-[2rem] border border-white/10 bg-white/[0.065] p-6 shadow-2xl shadow-black/20 backdrop-blur-2xl sm:p-7"><div className="mb-7 flex items-start justify-between gap-3"><div className="flex items-center gap-3"><AssetIcon asset={bitcoin} large /><div><p className="font-bold">بیت‌کوین</p><p className="text-xs text-slate-400">BTC / USDT</p></div></div><span className="rounded-full border border-cyan-300/20 bg-cyan-300/10 px-2.5 py-1 text-[10px] text-cyan-200">{bitcoin.sources.price}</span></div><p className="mb-1 text-xs text-slate-400">قیمت لحظه‌ای</p><div className="flex items-end justify-between gap-3"><p dir="ltr" className="text-3xl font-black tracking-tight">{formatUsd(bitcoin.priceUsd)}</p><span dir="ltr" className={`mb-1 flex items-center gap-1 text-sm font-bold ${bitcoin.change24h >= 0 ? 'text-emerald-300' : 'text-rose-300'}`}>{bitcoin.change24h >= 0 ? <ArrowUpLeft className="h-4 w-4" /> : <ArrowDownLeft className="h-4 w-4" />}{bitcoin.change24h >= 0 ? '+' : ''}{faNumber.format(bitcoin.change24h)}%</span></div><p className="mt-3 text-xs text-slate-400">{bitcoin.priceIrt ? `${faMoney.format(bitcoin.priceIrt)} تومان` : 'قیمت تومانی در دسترس نیست'}</p><div className="mt-5 flex items-end justify-between border-t border-white/10 pt-4"><div className="text-xs text-slate-400"><p>ارزش بازار</p><strong className="mt-1 block text-sm text-slate-200">{formatCompactUsd(bitcoin.marketCap)}</strong></div><Sparkline values={bitcoin.sparkline} positive={bitcoin.change24h >= 0} /></div></div>}
          </section>

          <section className="mb-6 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {[{ label: 'ارزش کل بازار', value: formatCompactUsd(market.global.marketCap), note: `${market.global.marketCapChange24h >= 0 ? '+' : ''}${faNumber.format(market.global.marketCapChange24h)}٪ امروز`, icon: Globe2 }, { label: 'حجم معاملات ۲۴ ساعت', value: formatCompactUsd(market.global.volume24h), note: `${faNumber.format(market.global.activeCryptocurrencies || 0)} ارز فعال`, icon: BarChart3 }, { label: 'دامیننس بیت‌کوین', value: `${faNumber.format(market.global.btcDominance)}٪`, note: market.global.source, icon: Bitcoin }, { label: 'ارزهای صعودی', value: `${faNumber.format(marketUp)} از ${faNumber.format(assets.length)}`, note: 'در فهرست منتخب', icon: TrendingUp }].map((stat) => { const Icon = stat.icon; return <div key={stat.label} className="rounded-3xl border border-white/10 bg-white/[0.05] p-4 backdrop-blur-xl transition hover:-translate-y-0.5 hover:bg-white/[0.08]"><div className="mb-4 flex items-center justify-between"><span className="rounded-xl bg-white/[0.08] p-2 text-cyan-200"><Icon className="h-4 w-4" /></span><span className="text-[10px] text-slate-500">زنده</span></div><p className="text-xs text-slate-400">{stat.label}</p><p dir="ltr" className="mt-1 text-lg font-bold text-slate-100">{stat.value}</p><p className="mt-2 text-[11px] font-semibold text-emerald-300">{stat.note}</p></div>; })}
          </section>
          ...
          <section className="rounded-[2rem] border border-white/10 bg-white/[0.055] p-4 shadow-2xl shadow-black/20 backdrop-blur-2xl sm:p-6">
            <div className="mb-6 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-center gap-3"><div className="rounded-2xl bg-gradient-to-br from-violet-300 to-indigo-600 p-2.5 text-slate-950"><LayoutGrid className="h-5 w-5" /></div><div><h2 className="text-lg font-black">۱۵۰ ارز برتر بازار</h2><p className="mt-1 text-xs text-slate-400">جستجو در نام و نماد؛ برای تحلیل روی هر ارز بزنید</p></div></div><div className="relative w-full max-w-xs"><Search className="absolute right-3 top-1/2 h-4 w-4 -translate-y-1/2 text-slate-500" /><input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="جستجوی هر ارز، نام یا نماد" className="h-10 w-full rounded-2xl border border-white/10 bg-black/15 pr-10 pl-3 text-sm text-white outline-none placeholder:text-slate-500 focus:border-cyan-300/50" /></div></div>
            <div className="mb-5 flex items-center gap-2 overflow-x-auto pb-1 scrollbar-hide">{filters.map((filter) => <button key={filter.id} type="button" onClick={() => setActiveFilter(filter.id)} className={`whitespace-nowrap rounded-full px-4 py-2 text-xs font-semibold transition ${activeFilter === filter.id ? 'bg-white text-slate-950' : 'border border-white/10 bg-white/[0.04] text-slate-400 hover:text-white'}`}>{filter.label}</button>)}<span className="mr-auto whitespace-nowrap text-[11px] text-slate-500">{faNumber.format(visibleAssets.length)} نتیجه · مرتب‌سازی: {sortLabels[sort.key]}</span></div>
            <div className="overflow-x-auto"><table className="w-full min-w-[1040px] border-separate border-spacing-y-2 text-right"><thead><tr className="text-[11px] text-slate-500"><th className="px-4 pb-2 font-medium"><SortButton label="دارایی" active={sort.key === 'name' || sort.key === 'rank'} direction={sort.direction} onClick={() => toggleSort(sort.key === 'name' ? 'rank' : 'name')} /></th><th className="px-4 pb-2 font-medium"><SortButton label="قیمت جهانی" active={sort.key === 'priceUsd'} direction={sort.direction} onClick={() => toggleSort('priceUsd')} /></th><th className="px-4 pb-2 font-medium"><SortButton label="قیمت تومان" active={sort.key === 'priceIrt'} direction={sort.direction} onClick={() => toggleSort('priceIrt')} /></th><th className="px-4 pb-2 font-medium"><SortButton label="تغییر ۲۴ ساعت" active={sort.key === 'change24h'} direction={sort.direction} onClick={() => toggleSort('change24h')} /></th><th className="px-4 pb-2 font-medium"><SortButton label="تغییر ۷ روز" active={sort.key === 'change7d'} direction={sort.direction} onClick={() => toggleSort('change7d')} /></th><th className="px-4 pb-2 font-medium">نمودار ۷ روز</th><th className="px-4 pb-2 font-medium"><SortButton label="حجم معاملات" active={sort.key === 'volume24h'} direction={sort.direction} onClick={() => toggleSort('volume24h')} /></th><th className="px-4 pb-2 font-medium"><SortButton label="ارزش بازار" active={sort.key === 'marketCap'} direction={sort.direction} onClick={() => toggleSort('marketCap')} /></th></tr></thead><tbody>{visibleAssets.map((asset) => { const positive = asset.change24h >= 0; const favorite = favorites.includes(asset.id); return <tr key={asset.id} className="group bg-white/[0.035] text-sm transition hover:bg-white/[0.09]"><td className="rounded-r-2xl px-4 py-3"><div className="flex items-center gap-3"><button type="button" onClick={(event) => { event.preventDefault(); toggleFavorite(asset.id); }} className="text-slate-600 hover:text-amber-300" aria-label={favorite ? 'حذف از علاقه‌مندی‌ها' : 'افزودن به علاقه‌مندی‌ها'}><Star className={`h-4 w-4 ${favorite ? 'fill-amber-300 text-amber-300' : ''}`} /></button><Link href={`/crypto-prices/${encodeURIComponent(asset.id)}`} className="flex min-w-0 items-center gap-3 rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300"><AssetIcon asset={asset} /><div><p className="font-bold text-slate-100 transition group-hover:text-cyan-200">{assetDisplayName(asset)}</p><p dir="ltr" className="mt-0.5 text-[11px] text-slate-500">{asset.symbol} · #{asset.rank}</p></div></Link></div></td><td dir="ltr" className="px-4 py-3 font-bold text-slate-100">{formatUsd(asset.priceUsd)}<span className="mt-1 block text-[9px] font-normal text-slate-600">{asset.sources.price}</span></td><td className="px-4 py-3 font-bold text-slate-200">{asset.priceIrt ? faMoney.format(asset.priceIrt) : '—'}<span className="mt-1 block text-[9px] font-normal text-slate-600">{asset.priceIrt ? 'تومان' : 'نوبیتکس'}</span></td><td dir="ltr" className={`px-4 py-3 font-semibold ${positive ? 'text-emerald-300' : 'text-rose-300'}`}>{positive ? '+' : ''}{faNumber.format(asset.change24h)}%</td><td dir="ltr" className={`px-4 py-3 font-semibold ${asset.change7d >= 0 ? 'text-emerald-300' : 'text-rose-300'}`}>{asset.change7d >= 0 ? '+' : ''}{faNumber.format(asset.change7d)}%</td><td className="px-4 py-3"><Sparkline values={asset.sparkline} positive={asset.change7d >= 0} /></td><td className="px-4 py-3 text-xs text-slate-400">{formatCompactUsd(asset.volume24h)}</td><td className="rounded-l-2xl px-4 py-3 text-xs text-slate-400"><Link href={`/crypto-prices/${encodeURIComponent(asset.id)}`} className="transition hover:text-cyan-200">{formatCompactUsd(asset.marketCap)}</Link></td></tr>; })}</tbody></table>{visibleAssets.length === 0 && <div className="rounded-2xl border border-dashed border-white/10 py-12 text-center text-sm text-slate-400">ارزی با این نام یا نماد در ۱۵۰ ارز برتر پیدا نشد.</div>}</div>
          </section>

          <div className="mt-5 flex flex-col items-center justify-between gap-3 text-[11px] text-slate-500 sm:flex-row"><p className="flex items-center gap-1.5"><Clock3 className="h-3.5 w-3.5" />آخرین به‌روزرسانی: {new Date(market.generatedAt).toLocaleTimeString('fa-IR', { hour: '2-digit', minute: '2-digit' })}</p><p className="flex items-center gap-1.5"><WalletCards className="h-3.5 w-3.5" />این صفحه توصیه سرمایه‌گذاری نیست.</p></div>
        </>}
      </div>
    </main>
  );
