...
function Change({ value }: { value: number }) {
  ...
  return <span dir="ltr" className={`inline-flex items-center gap-1 font-bold ${positive ? 'text-success' : 'text-destructive'}`}>{positive ? <ArrowUpLeft className="h-4 w-4" /> : <ArrowDownLeft className="h-4 w-4" />}{positive ? '+' : ''}{fa.format(value)}%</span>;
}
function Metric({ label, value, note, icon: Icon }: { label: string; value: string; note?: string; icon: typeof Coins }) {
  return <div className="rounded-3xl border border-border/60 bg-card/90 p-5 backdrop-blur-xl"><div className="mb-4 flex items-center justify-between"><Icon className="h-5 w-5 text-primary" /><span className="text-[10px] text-muted-foreground">داده بازار</span></div><p className="text-xs text-muted-foreground">{label}</p><p dir="ltr" className="mt-2 text-lg font-black text-foreground">{value}</p>{note && <p className="mt-2 text-[11px] text-muted-foreground">{note}</p>}</div>;
}
...
export default function CryptoAssetAnalysisPage({ id }: { id: string }) {
  ...
  return <main className="public-page-shell relative min-h-screen overflow-hidden bg-background pb-14 pt-20 text-foreground md:pt-28" dir="rtl">
    ...
    <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-10">
      <Link href="/crypto-prices" className="mb-6 inline-flex items-center gap-2 rounded-full border border-border/60 bg-card/90 px-4 py-2 text-xs text-muted-foreground transition hover:bg-card/10 hover:text-foreground"><ArrowRight className="h-4 w-4" />بازگشت به بازار</Link>
      {loading && !data ? <div className="flex min-h-[500px] items-center justify-center"><Loader2 className="h-9 w-9 animate-spin text-primary" /></div> : error && !data ? <div className="rounded-3xl border border-destructive/20 bg-destructive/10 p-12 text-center"><p className="text-destructive">{error}</p><button onClick={load} className="mt-5 rounded-xl bg-card px-4 py-2 text-sm font-bold text-foreground">تلاش دوباره</button></div> : asset && data ? <>
        <section className="mb-5 grid gap-5 xl:grid-cols-[1.2fr_.8fr]">
          <div className="relative overflow-hidden rounded-[2rem] border border-border/10 bg-gradient-to-br from-card via-card to-muted/50 p-6 shadow-2xl sm:p-9"><div className="flex flex-col gap-8 sm:flex-row sm:items-start sm:justify-between"><div className="flex items-center gap-4">{asset.imageUrl ? <Image src={asset.imageUrl} alt={asset.name} width={64} height={64} unoptimized className="h-16 w-16 rounded-2xl bg-card/10 p-1" /> : <span className="flex h-16 w-16 items-center justify-center rounded-2xl bg-primary text-xl font-black text-primary-foreground">{asset.symbol[0]}</span>}<div><div className="flex items-center gap-2"><h1 className="text-3xl font-black sm:text-4xl">{asset.name}</h1><span className="rounded-full bg-card/10 px-2 py-1 text-xs text-muted-foreground">#{asset.rank}</span></div><p dir="ltr" className="mt-2 text-sm text-muted-foreground">{asset.symbol} / USD</p></div></div><button onClick={load} className="flex items-center gap-2 self-start rounded-full border border-border/10 px-3 py-2 text-xs text-muted-foreground hover:bg-card/10"><RefreshCw className="h-3.5 w-3.5" />به‌روزرسانی</button></div><div className="mt-10 flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><p className="text-xs text-muted-foreground">قیمت لحظه‌ای</p><p dir="ltr" className="mt-2 text-4xl font-black sm:text-5xl">{formatUsd(asset.priceUsd)}</p><p className="mt-3 text-sm text-muted-foreground">{asset.priceIrt ? `${faMoney.format(asset.priceIrt)} تومان` : 'قیمت تومانی در دسترس نیست'}</p></div><div className="rounded-2xl border border-border/10 bg-background/15 px-4 py-3"><p className="mb-2 text-[10px] text-muted-foreground">تغییر ۲۴ ساعت</p><Change value={asset.change24h} /></div></div></div>
          <div className="rounded-[2rem] border border-border/60 bg-card/90 p-6 backdrop-blur-2xl"><div className="mb-6 flex items-center justify-between"><div><p className="text-xs text-muted-foreground">مومنتوم بازار</p><h2 className="mt-1 text-xl font-black">نمای تغییرات</h2></div><TrendingUp className="h-6 w-6 text-primary" /></div><div className="grid grid-cols-3 gap-3">{[{ label: '۲۴ ساعت', value: asset.change24h }, { label: '۷ روز', value: asset.change7d }, { label: '۳۰ روز', value: asset.change30d }].map((item) => <div key={item.label} className="rounded-2xl bg-muted/50 p-3 text-center"><p className="mb-2 text-[10px] text-muted-foreground">{item.label}</p><Change value={item.value} /></div>)}</div><div className="mt-7"><div className="mb-3 flex justify-between text-[11px] text-muted-foreground"><span>کف ۲۴ ساعت {formatUsd(asset.low24h)}</span><span>سقف {formatUsd(asset.high24h)}</span></div><div className="relative h-2 rounded-full bg-card/10"><div className="absolute inset-y-0 right-0 rounded-full bg-gradient-to-l from-primary to-accent" style={{ width: `${rangePosition}%` }} /><span className="absolute top-1/2 h-4 w-4 -translate-y-1/2 rounded-full border-2 border-background bg-card" style={{ right: `calc(${rangePosition}% - 8px)` }} /></div></div></div>
        </section>
        <section className="mb-5 rounded-[2rem] border border-border/60 bg-card/90 p-4 sm:p-6"><div className="mb-5 flex items-center justify-between"><div><h2 className="text-xl font-black">نمودار هفت‌روزه</h2><p className="mt-1 text-xs text-muted-foreground">روند قیمت بر پایه داده‌های بازار</p></div><BarChart3 className="h-5 w-5 text-primary" /></div><div className="h-72 w-full overflow-hidden rounded-2xl bg-background/10 p-3"><svg viewBox="0 0 1000 260" preserveAspectRatio="none" className="h-full w-full"><defs><linearGradient id="detail-line" x1="0" y1="0" x2="1" y2="0"><stop stopColor="hsl(var(--primary))" /><stop offset="1" stopColor="hsl(var(--success))" /></linearGradient></defs><path d={path(asset.sparkline)} fill="none" stroke="url(#detail-line)" strokeWidth="4" strokeLinecap="round" strokeLinejoin="round" /></svg></div></section>
