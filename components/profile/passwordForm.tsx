...
const PasswordForm = forwardRef((props, ref) => {
  useImperativeHandle(ref, () => ({
    submit: () => {
      // در اینجا می‌توانید منطق تغییر رمز عبور (مثلاً فراخوانی API) را پیاده کنید.
      console.log("PasswordForm submitted.");
    },
  }));
  ...
  return (
    <div className="bg-[#fafafa] w-full rounded mt-8">
      ...
      {/* دکمه تغییر رمز عبور */}
      <div className="p-5">
        <Button className="text-xs px-8">تغییر رمز عبور</Button>
      </div>
    </div>
  );
});
