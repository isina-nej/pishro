      ...
      if (category.statsBoxes) {
        parsedStatsBoxes =
          typeof category.statsBoxes === "string"
            ? (JSON.parse(category.statsBoxes) as StatsBox[])
            : (category.statsBoxes as StatsBox[]);
      }
    } catch (e) {
      console.error("Error parsing statsBoxes:", e);
    }

    const landingContent = category.content.find(
      (c: PageContent) => c.type === "LANDING"
    );
    const aboutContent = category.content.find(
      (c: PageContent) => c.type === "ABOUT"
    );
