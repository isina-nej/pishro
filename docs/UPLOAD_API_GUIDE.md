...
## 📚 آپلود فایل کتاب، صوت و تصویر جلد (ادمین)

### `POST /api/admin/books/upload`

**کاربرد:** آپلود فایل کتاب (PDF/EPUB)، فایل صوتی و همچنین کاور کتاب برای استفاده در کالکشن دیجیتال بوک‌ها
...
| نام پارامتر | نوع | الزامی | توضیحات |
|------------|-----|--------|---------|
| `book` | File | ❌ | فایل کتاب (PDF یا EPUB) |
| `audio` | File | ❌ | فایل صوتی (MP3, M4A, WAV, OGG) |
| `cover` | File | ❌ | تصویر کاور کتاب |

**محدودیت‌ها و فرمت‌ها:**

- حجم فایل کتاب: حداکثر **100MB**
- حجم فایل صوتی: حداکثر **200MB**
- حجم تصویر کاور: حداکثر **10MB**
- فرمت‌های فایل کتاب: **PDF, EPUB** (MIME: application/pdf, application/epub+zip)
- فرمت‌های صوتی: **MP3, M4A, WAV, WEBM, OGG**
- فرمت‌های تصویر: **JPG, JPEG, PNG, GIF, WEBP, SVG**

**مکان ذخیره فایل:**

- به‌صورت پیش‌فرض فایل‌ها در مسیر `public/uploads/books/{files|audios|covers}` ذخیره می‌شوند.
- برای محیط‌های Production ممکن است از `UPLOAD_STORAGE_PATH` یا Object Storage (S3) استفاده شود؛ در این صورت آدرس نسبی بازگردانده‌شده باید با `UPLOAD_BASE_URL` ترکیب شود.
...
```json
{
  ...
  "data": {
    "uploads": [
      ...
      { "field": "cover", "url": "/uploads/books/covers/cover_12345.jpg" }
    ]
  }
}
```

بعد از دریافت URL، از این مقادیر برای فیلدهای `fileUrl`, `audioUrl`, و `cover` در ایجاد یا ویرایش کتاب (endpoint `/api/admin/books`) استفاده کنید.

**نمونه درخواست با curl:**

```bash
curl -X POST "https://your-domain.com/api/admin/books/upload" \
...
در پاسخ، `data.uploads` شامل URL های فایل‌های آپلود شده خواهد بود.
